...
async function initializeServer() {
  ...
  // Check server status (max 10 seconds)
  let attempts = 0;
  ...
  while (attempts < maxAttempts) {
    ...
    if (attempts < maxAttempts) {
      ...
      setStatus('initializing', `Connecting... ${attempts}s`);
    }
  }
  
  // Server failed to start - still allow chat, just with warning
  serverReady = false;
  setStatus('error', 'Server offline');
  addMessage('error', '⚠️ Server still loading. You can try sending a message and it will retry automatically.');
}
